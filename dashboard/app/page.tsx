...
export default async function OverviewPage() {
  ...
  const goedgekeurd = calls?.filter((c: Call) => c.risk_level === 'GOEDGEKEURD').length ?? 0
  const afgekeurd = calls?.filter((c: Call) => c.risk_level === 'AFGEKEURD').length ?? 0
  const risico = calls?.filter((c: Call) => c.risk_level === 'RISICO').length ?? 0
